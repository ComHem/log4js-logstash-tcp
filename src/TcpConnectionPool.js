...
class TcpConnectionWrapper extends EventEmitter {
  constructor(host, port, config) {
    ...
    this.connectionNotReadyRetryInterval = parseInt(
      config.connectionNotReadyRetryInterval || 500
    );
    ...
    this.connection.on("error", (err) => {
      console.error(
        `logstash Error with the connection to ${host}:${port}. ${err}`
      );
...
class TcpConnectionPool {
  ...
  _getTcpConnection(host, port) {
    ...
    return (
      this.tcpConnections[index] || this._createTcpConnection(index, host, port)
    );
  }
  ...
  _createIndexForTcpConnection() {
    return (
      Math.floor(Math.random() * this.config.maxTcpConnections) %
      this.config.maxTcpConnections
    );
  }
}
